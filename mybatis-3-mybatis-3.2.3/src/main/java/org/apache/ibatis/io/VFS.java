 ...
 * <p>
 * VFS 主要用来加载容器内的各种资源，比如jar或者class文件。mybatis提供了两个实现 {@link DefaultVFS} 和 {@link JBoss6VFS}，并提供了用户扩展点，用于自定义VFS实现，
 * 加载顺序是自定义VFS实现 > 默认VFS实现
 * 取第一个加载成功的，默认情况下会先加载JBoss6VFS，如果 classpath下找不到jboss的vfs实现才会加载VFS实现。
 *
 * @author Ben Gunter
 */
